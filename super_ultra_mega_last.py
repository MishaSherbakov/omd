...
async def game(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Проверка, выбрана ли свободная ячейка
    if keyboard_state[row][col] == SPACE:
        ...
        if won([keyboard_state[r][c] for r in range(3) for c in range(3)]):
            await query.answer(f'Поздравляю! Вы выиграли, {user.first_name}!', show_alert=True)
            keyboard = generate_keyboard(keyboard_state)  # Обновление клавиатуры последним ходом
            reply_markup = InlineKeyboardMarkup(keyboard)
            await query.edit_message_text(f'Х (ваш) ход! Пожалуйста, поставьте Х на свободное место', reply_markup=reply_markup)
            return end(update, context)  # Вызов функции завершения, когда пользователь выигрывает
            ...
            reply_markup = InlineKeyboardMarkup(keyboard)
            await query.edit_message_text(f'Х (ваш) ход! Пожалуйста, поставьте Х на свободное место', reply_markup=reply_markup)
            return end(update, context)  # Вызов функции завершения, когда игра ничья
        else:
            ...
            if empty_cells:
                ...
                if won([keyboard_state[r][c] for r in range(3) for c in range(3)]):
                    await query.answer(f'Извините, {user.first_name}. Вы проиграли. Удачи в следующий раз!', show_alert=True)
                    keyboard = generate_keyboard(keyboard_state)  # Обновление клавиатуры последним ходом
                    reply_markup = InlineKeyboardMarkup(keyboard)
                    await query.edit_message_text(f'Х (ваш) ход! Пожалуйста, поставьте Х на свободное место', reply_markup=reply_markup)
                    return end(update, context)  # Вызов функции завершения, когда противник выигрывает
                ...
                # Нет свободных ячеек, игра ничья
                await query.answer('Ничья!', show_alert=True)
                keyboard = generate_keyboard(keyboard_state)  # Обновление клавиатуры последним ходом
                reply_markup = InlineKeyboardMarkup(keyboard)
                await query.edit_message_text(f'Х (ваш) ход! Пожалуйста, поставьте Х на свободное место', reply_markup=reply_markup)
